  ...
  <div class="cart">
      ...
      <div class="cart-body">
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con7">
            <a class="sindelet" @click="deleteCarId(cart)">删除</a>
            <br>
            <a href="#none">移到收藏</a>
          </li>
        </ul>
    ...
    <div class="cart-tool">
      <div class="select-all">
        <input class="chooseAll" type="checkbox" @change="updateAllCar">
        <span>全选</span>
      </div>
      <div class="option">
  ...
  export default {
    ...
    methods: {
     ...
     async  handler(type,disNum,cart){
        ...
        //  this.$store.dispatch('addShopCart',{skuId:cart.skuId,skuNum:disNum})
        try {
   ...
   async updateAllCar(event){
     try {
        let isChecked =event.target.checked?'1':'0';
      // console.log(checked);
    await  this.$store.dispatch('updateAllCarList',isChecked);
       this.getDate();
     } catch (error) {
       alert(error.message);
     }
